...
class SWE_Pooling_AxisAligned(nn.Module):
    def __init__(self, d_in, num_ref_points, learned_ref=True, flatten_embeddings=True):
        '''
        ...
        super().__init__()
        self.d_in = d_in
        self.num_slices = d_in # axis-aligned slices
        self.num_ref_points = num_ref_points
        self.learned_ref = learned_ref
        self.flatten_embeddings = flatten_embeddings

        if self.learned_ref: # whether we are training a reference; otherwise, the reference should be provided as an input argument in the forward pass
            uniform_ref = torch.linspace(-1, 1, num_ref_points).unsqueeze(1).repeat(1, d_in)
            self.reference = nn.Parameter(uniform_ref) # initalize the M x d_in reference using a uniform distribution 
    ...
    def forward(self, X, mask=None, ref=None):#, flatten=True):
        '''
        ...
        Input:
            ...
            # TODO
        Output:
            weighted_embeddings: B x num_slices tensor, containing a batch of B pooled embeddings, each of dimension "num_slices" (i.e., number of slices) # TODO
        '''
        ...
        if mask is None:
            ...
            if N == self.num_ref_points:
                ...
                xnew = torch.linspace(0, 1, self.num_ref_points + 2)[1:-1].unsqueeze(0).repeat(B * self.num_slices, 1).to(X.device)
        ...
        if ref is None:
            assert self.learned_ref == True # if no reference is provided, we should be learning it
            Rslices = self.get_slice(self.reference).expand(Xslices_sorted_interpolated.shape) # B x M x num_slices
        else:
            Rslices = self.get_slice(ref) # B x M x num_slices

        _, Rind = torch.sort(Rslices, dim=1)
        embeddings = (Rslices - torch.gather(Xslices_sorted_interpolated, dim=1, index=Rind)) # B x M x num_slices

        if not self.flatten_embeddings:
            return embeddings
        else:
            weighted_embeddings = self.weight(embeddings.permute(0, 2, 1)).sum(-1)
    ...
    def get_slice(self, X):
        ...
        '''
        return X # axis-aligned (i.e., identity) slices
